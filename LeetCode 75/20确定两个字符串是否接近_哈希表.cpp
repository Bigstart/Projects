...
bool closeStrings(std::string word1, std::string word2) {
    ...
    for (char c : word2) {
        ...
        if (freq1.find(c) == freq1.end()) {
            return false; // 如果字符种类不同，返回false
        }
    }

    std::vector<int> freqCount1, freqCount2;
    for (auto& pair : freq1) {
    ...
    std::sort(freqCount1.begin(), freqCount1.end());
    std::sort(freqCount2.begin(), freqCount2.end());
...
int main() {
    ...
    std::cout << "Are the words close? " << std::boolalpha << result << std::endl;
