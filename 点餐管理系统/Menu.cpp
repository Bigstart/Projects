...
{
	if (date2.year == year && date2.month == month && date2.day == day && date2.hour == hour)
	{
		return true;
	}
	return false;
...
Order::Order(string dishName, string dishID, double price, string customerName, string adress, string phone, Date bookDate)
{
...
const bool Order::operator==(Order order2)
{
	...
	return true;// 如果所有属性都相等，返回 true，表示两个对象相等
}
